...
pub fn update_build_guide(
        ...
        || !demolish_events.is_empty()
    {
        ...
        {
            let template = building_type.template(tile_pos);
            ...
            let is_belt_edit = if let BuildingType::Belt = building_type 
                && let Ok(tile_entity) = map_query.get_tile_entity(tile_pos, active_map.map_id, MapLayer::Buildings)
        ...
        || !demolish_events.is_empty()
    {
            ...
            && map_interaction.is_allowed()
        {
            ...
            let demolished_building = buildings
                .iter()
                .find_map(|b| b.tiles.iter().any(|(pos, _)| *pos == tile_pos).then_some(b));

            if let Some(building) = demolished_building {
                for (pos, e) in building.tiles.iter() {
                    if let Ok(mut tile) = tiles.get_mut(*e) {
